...
// export function GetTypedCandidate(candidate: MSLCandidate | FireBaseCandidate){
//     if (typeof candidate.Post === 'object') return candidate as MSLCandidate;
//     return candidate as FireBaseCandidate;
// }
...
export function regReplace(post: string, group: regexReplaceI) {
    group.regex.forEach((rgx, i) => {
        ...
        post = post.replace(rgx2, group.replace[i]).trim();
    })
    return post;
}
